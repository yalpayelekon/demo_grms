...
class _ReportsPageState extends ConsumerState<ReportsPage> {
  ...
  void initState() {
    super.initState();
    // Initialize with today or some defaults if needed
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildReportRow(context, reportsService, ReportType.service, 'Service Reports', 'Room service status and workflow statistics', '🛎️'),
            const SizedBox(height: 16),
            _buildReportRow(context, reportsService, ReportType.energy, 'Energy Reports', 'Analysis of energy usage across all rooms', '📊'),
            const SizedBox(height: 16),
            _buildReportRow(context, reportsService, ReportType.activity, 'Activity Reports', 'Combined alarm and room service history', '🗒️'),
          ],
        ),
      ),
    );
  ...
  Widget _buildReportRow(BuildContext context, ReportsService service, ReportType type, String title, String subtitle, String icon) {
    return Card(
      elevation: 0,
          ...
          children: [
            ...
            Expanded(
              ...
              child: Row(
                children: [
                  _DateButton(
                    label: _startDates[type] == null ? 'Start Date' : DateFormat('yyyy-MM-dd').format(_startDates[type]!),
                    onTap: () async {
                      final picked = await showDatePicker(context: context, initialDate: _startDates[type] ?? DateTime.now(), firstDate: DateTime(2020), lastDate: DateTime.now());
                      if (picked != null) setState(() => _startDates[type] = picked);
                    },
                  ),
                  const Padding(padding: EdgeInsets.symmetric(horizontal: 8), child: Text('-', style: TextStyle(color: Colors.grey))),
                  _DateButton(
                    label: _endDates[type] == null ? 'End Date' : DateFormat('yyyy-MM-dd').format(_endDates[type]!),
                    onTap: () async {
                      final picked = await showDatePicker(context: context, initialDate: _endDates[type] ?? DateTime.now(), firstDate: DateTime(2020), lastDate: DateTime.now());
                      if (picked != null) setState(() => _endDates[type] = picked);
                    },
                  ),
                ],
              ),
  ...
  Widget _buildActionButtons(BuildContext context, ReportsService service, ReportType type) {
    return Row(
      children: [
        _SmallActionBtn(label: 'CSV', color: Colors.green, onTap: () => _handleExport(service, type, 'csv')),
        const SizedBox(width: 8),
        _SmallActionBtn(label: 'PDF', color: Colors.red, onTap: () => _handleExport(service, type, 'pdf')),
        const SizedBox(width: 8),
        _SmallActionBtn(label: 'XLS', color: Colors.blue, onTap: () => _handleExport(service, type, 'xls')),
  ...
  Future<void> _handleExport(ReportsService service, ReportType type, String format) async {
    ...
    if (format == 'csv' || format == 'xls') {
      ...
      final filename = format == 'csv' ? "${report.filename}.csv" : "${report.filename}.xls";
  ...
  void _showPreview(BuildContext context, ReportsService service, ReportType type) {
    final report = service.buildReport(type, _startDates[type], _endDates[type]);
    if (report == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select valid dates')));
      return;
            ...
            children: [
                ...
                child: Row(
                  ...
                  children: [
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Report Preview', style: TextStyle(color: Colors.grey, fontSize: 12)),
              ...
              Expanded(
                child: report.rows.isEmpty 
                  ? const Center(child: Text('No records found for the selected range.'))
                  : SingleChildScrollView(
                      ...
                      child: Table(
                        ...
                        children: [
                          TableRow(
                            decoration: BoxDecoration(color: Colors.indigo.withOpacity(0.3)),
                            children: report.headers.map((h) => Padding(
                              padding: const EdgeInsets.all(12),
                          ...
                          ...report.rows.map((row) => TableRow(
                            children: row.map((cell) => Padding(
                              padding: const EdgeInsets.all(12),
                              child: Text(cell.toString(), style: const TextStyle(fontSize: 12)),
                            )).toList(),
                          )),
                        ],
                      ),
      ...
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          pw.Table.fromTextArray(
            ...
            data: report.rows.map((row) => row.map((e) => e.toString()).toList()).toList(),
